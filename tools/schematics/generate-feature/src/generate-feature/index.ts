...
function parseModel( tree: Tree,options:any ): Record<string, string> {
 
  const content = tree.read(options.model)
const modelpath =options.model 
  if (!content) throw new Error(`Model file not found options.model: ${options.model},`);
...
export function generateFeature(_options: any): Rule {
  return (_: Tree, _context: SchematicContext) => {
    ...
    const modelProps = _options.model ? parseModel(_, _options) : {};
