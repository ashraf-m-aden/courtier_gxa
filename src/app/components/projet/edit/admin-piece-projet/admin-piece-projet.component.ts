...
@Component({
  selector: 'edit-admin-piece-projet',
  imports: [FormsModule, ReactiveFormsModule, DatePipe],
  templateUrl: './admin-piece-projet.component.html',
  styleUrl: './admin-piece-projet.component.css'
})
export class EditAdminPieceProjetComponent {
  ...
  constructor(private fb: FormBuilder, private courtierService: CourtierService, private toastr: ToastrService) {
    effect(() => {
      this.pieces = this.retrievedproject().filter((c: any) => { return c.typename == "piec" })
      console.log("UPDATE");
    ...
    this.pieceForm = this.fb.group({
      ...
      Contrat: [this.project()?.Contrat || 0],
      Piece: [0],
      Adhprin: [0],
      Codeprod: [""],
      Oripiece: [null],
  ...
  ngOnChanges(): void {
    ...
    console.log(this.retrievedproject());
  ...
  onSubmit() {
    if (this.pieceForm.valid) {
      let cont = this.retrievedproject().filter((c: any) => { return c.typename == "CONT" })[0];
      cont.Echpjj = cont.Echpjj.toString()
      let payload = {
        // "contrat": this.project().Contrat,

        "dossier": this.project().Numtiers,
        "produit": this.project()?.Codeprod ?? this.pieces[0]?.Codeprod,
  ...
  onCancel() {
    ...
    this.pieceForm.get("Contrat")?.setValue(this.retrievedproject()[0].Contrat)
    this.pieceForm.get("Codeprod")?.setValue(this.retrievedproject()[0].Codeprod)
  }
}
