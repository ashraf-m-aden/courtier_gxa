...
@Component({
  selector: 'edit-produits-projet',
  imports: [FormsModule, ReactiveFormsModule, MatInputModule, MatSelectModule, DatePipe],
  templateUrl: './produits-projet.component.html',
  styleUrl: './produits-projet.component.scss'
})
export class EditProduitsProjetComponent {
  ...
  constructor(private fb: FormBuilder, private courtierService: CourtierService) {
    ...
    effect(() => {
      if (this.adhesion()?.Codeprod || this.project().Codeprod) {
        this.courtierService.getDetailProduit(this.adhesion()?.Codeprod ?? this.project().Codeprod).subscribe({
          next: (data: Produit) => {

            this.detailProduit.set(data);

            this.codeprodSignal.set(this.adhesion()?.Codeprod ?? this.project().Codeprod);
