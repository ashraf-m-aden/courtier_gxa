...
export class AdminPieceContratComponent {
  ...
  constructor(private fb: FormBuilder, private courtierService: CourtierService) {
    ...
    this.pieceForm = this.fb.group({
      ...
      Contrat: [this.contratDetails()[0].Contrat],
      Piece: [0],
      Adhprin: [0],
      Codeprod: [this.contratDetails()[0].Codeprod],
      Oripiece: [''],
      Sitpiece: [''],
      Datesit: [''],
      Suspens: [''],
  ...
  onSubmit() {
    if (this.pieceForm.valid) {
      ...
      if (payload.produit && payload.produit != "" && payload.produit != null && payload.produit != undefined) {
        this.courtierService.postUpdatePieceDuContrat(payload).subscribe({
          next: () => {
            this.courtierService.postDetailContrat(payload.contrat).subscribe({
              next: (data: any) => {
        ...
        this.courtierService.postAjoutPieceAuContrat(payload).subscribe({
          next: () => {
            this.courtierService.postDetailContrat(payload.contrat).subscribe({
              next: (data: any) => {
                this.contratDetails.set(data)
